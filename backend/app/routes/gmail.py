...
async def connect_gmail(
    ...
    DEVELOPMENT NOTE: If headers are missing in dev mode, auth.py now returns 
    a demo user. This allows unblocked testing of the OAuth flow.
    """
    logger.info("Connect Gmail hit. User ID: %s, Clerk ID: %s", current_user.id, current_user.clerk_id)
    ...
    logger.info("Generated Google OAuth URL: %s", url)
...
async def gmail_callback(
    ...
    """
    logger.info("Received Google OAuth callback. Code length: %d, State: %s", len(code), state)
    ...
    try:
        ...
        logger.info("Resolved user %s from OAuth state. Exchanging code for tokens...", user.id)

        # Exchange code for tokens and save
        oauth_service = GoogleOAuthService()
        token_record = await oauth_service.exchange_code_for_token(db, code, user.id)
        
        logger.info("Successfully linked Google OAuth token (ID: %s) for user %s", token_record.id, user.id)
        ...
        logger.error("Google OAuth callback failed: %s", str(e), exc_info=True)
